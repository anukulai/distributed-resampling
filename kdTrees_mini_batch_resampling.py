...
from src.sampling.mixed_sampling.dist_smogn_mini_kmeans_kdTrees import DistributedSMOGN_MiniKMeans_kdTrees
...
DATA_PROCESSED_DIR = f"{DATA_DIR}/processed_kdTrees"

RESULT_DIR = "kdTrees_results"
...
def run_smogn_with_n_partitions(n, label_col, train):
    ...
    try:
        start_time = time.time()
        train_dist_smogn_n = DistributedSMOGN_MiniKMeans_kdTrees(label_col=label_col, k_partitions=n).transform(train)
        end_time = time.time()
        time_taken = round(end_time - start_time, 3)

...
class DistributedSMOGN_faiss(BaseMixedSampler, _KMeansParams, _SMOGNParams):
    @keyword_only
    def __init__(self, label_col=None, sampling_strategy="balance", k_partitions=2, threshold=0.8, method="auto",
                 ...
                 perturbation=0.02):
        super(DistributedSMOGN_faiss, self).__init__()
    ...
    def _partition(self, df, partition_col):
        ...
        while redo_partition:
            ...
            feature_vectors = df_new[feature_vector_cols].values.astype(np.float32)

            k_means_object = faiss.Kmeans(
                d = len(feature_vector_cols),
                k = self.getKPartitions(),
                seed = random.randint(0, 1000)
            )
            k_means_object.train(feature_vectors)
            _, assigned_clusters = k_means_object.assign(feature_vectors)

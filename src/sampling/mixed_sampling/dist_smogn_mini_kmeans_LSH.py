...
class DistributedSMOGN_MiniKMeans_LSH(BaseMixedSampler, _KMeansParams, _SMOGNParams):
    @keyword_only
    def __init__(self, label_col=None, sampling_strategy="balance", k_partitions=2, threshold=0.8, method="auto",
                 ...
                 perturbation=0.02):
        super(DistributedSMOGN_MiniKMeans_LSH, self).__init__()
    ...
    def _oversample(self, bump):
        ...
        # return bump.samples.toPandas().groupby(partition_col).apply(create_synth_samples)  # change this to spark
    ...
    def _create_synth_samples(self, partition, cat_feature_cols, num_feature_cols, label_col, n_synth_samples, k,
        ...
        # instantiate LSHash
        lsh = LSHash(n_bits, n_dim)

        # index the vectors into lsh
        for idx in range(feature_vectors.shape[0]):
            lsh.index(feature_vectors[idx], extra_data=idx)
        ...
        for base_sample_index, base_sample in partition.iterrows():
            ...
            query_vector = base_sample[[*num_feature_cols]].to_numpy().reshape((1, n_dim))[0]

            nearest_neighbours = lsh.query(
                query_point=query_vector,
                num_results=k,
                distance_func="euclidean"
            )

            k = min(k, len(nearest_neighbours))
            ...
            for n_synth_sample in range(n_synth_samples):
                ...
                try:
                    ...
                    neighbour_sample_index = nearest_neighbours[random_num][0][1]

                    if neighbour_sample_index is None:
                        neighbour_sample_index = base_sample_index

                    neighbour_sample = partition.iloc[neighbour_sample_index]

                    dist_list = [nn[1] for nn in nearest_neighbours]
                    dist = dist_list[random_num]

                    if len(dist_list) == 1:
                        safe_dist = dist
                    else:
                        safe_dist = dist_list[(k + 1) // 2] / 2

...
from src.sampling.over_sampling.over_sampling_distadasyn import over_sampling_dist_adasyn
...
def partition_data(data, numerical_cols, num_partitions):
    feature_vectors = data[numerical_cols].values

    k_means_object = MiniBatchKMeans(
        n_clusters=num_partitions, tol=1e-4, max_iter=20,
    ...
    data["partition_id"] = assigned_clusters
    return data
...
########################################################################################
    n_dim = len(feat_list_num)
    n_bits = 10

    # instantiate LSHash
    lsh = LSHash(n_bits, n_dim)

    feature_vectors = data_num.to_numpy()

    # index the vectors into lsh
    for idx in range(feature_vectors.shape[0]):
        lsh.index(feature_vectors[idx], extra_data=idx)

    # prepare the knn_matrix -> n*k matrix
    knn_index = np.zeros(shape=(n, k+1))

    for idx in range(n):
        nn_result = lsh.query(query_point=feature_vectors[idx], num_results=k+1, distance_func="euclidean")
        for j in range(len(nn_result)):

            index_value = nn_result[j][0][1]
            knn_index[idx][j] = index_value

            if index_value is None:
                recovered_index = int(np.where(np.all(feature_vectors==nn_result[j][0][0], axis=1))[0])
                knn_index[idx][j] = recovered_index

    knn_index = knn_index.astype("int64")
    ...
    #         if feat_count_nom == 0:
    #             dist_matrix[i][j] = euclidean_dist(     ##  Anushree -> make change here
    #                 a=data_num.iloc[i],
    #                 b=data_num.iloc[j],
    #                 d=feat_count_num
    #             )
    ...
    for i in range(n_bumps):
        ...
        current_bump_index = b_index[i].index
        current_bump_df = data_orig.iloc[current_bump_index]

        ## no sampling
        if s_perc[i] <= 1:
            ## simply return no sampling
            ## results to modified training set
            data_new = pd.concat([current_bump_df, data_new],
                                 ignore_index=True)  # ????? verify if data_orig matches columns UPDATE: semi-verified

        ## over-sampling
        if s_perc[i] > 1:     ## Anushree -> probably implement KMeans here.
            ...
            partitioned_bump_df = partition_data(data=current_bump_df, numerical_cols=feat_list_num, num_partitions=2)

            ##################################################################

            # partition = partitioned_bump_df[partitioned_bump_df["partition_id"] == 0]  # remove it
            # # partitioned_bump_df.groupby(partition_id).applyInPandas(create_synth_samples, schema=schema)
            # # partitioned_bump_df.groupby(partition_id).apply(partitioned_bump_df)

            def create_synth_samples(partition):
                synth_obs = over_sampling_dist_adasyn(    # wrap this in a function that accepts a partition
                    partition=partition,
                    data=data,
                    label=label,
                ...
                return pd.DataFrame(data=synth_obs)
            # master_df = something.groupby(some_column).apply(some_functionn)


            spark_df = SPARK.createDataFrame(partitioned_bump_df)
            schema = spark_df.schema

            # set the new schema here to prevent partioton id bug
            spark_df_without_partition_id = spark_df.drop('partition_id')
            schema=spark_df_without_partition_id.schema
            # oversampled_df = partitioned_bump_df.groupby(['partition_id']).apply(create_synth_samples)
            oversampled_spark_df = spark_df.groupby(['partition_id']).applyInPandas(create_synth_samples, schema=schema)
            ## concatenate over-sampling
            ## results to modified training set
            oversampled_df = oversampled_spark_df.toPandas()
            oversampled_df.columns = [int(col) for col in oversampled_df.columns]
            data_new = pd.concat([oversampled_df, data_new], ignore_index=True)

...
def over_sampling_dist_adasyn(
    ...
    # partition_pd = pd.DataFrame(partition.collect(), columns=partition.columns)

    # partition=partition.toPandas()
    n = partition.shape[0]
    d = len(data.columns)
    index = list(partition.index)

    from pyspark.sql.functions import monotonically_increasing_id

    # partition = partition.withColumn("id", monotonically_increasing_id())
    # n = partition.count()
    # d = len(data.columns)
    # index = partition.select("id").rdd.flatMap(lambda x: x).collect()

    ## find knn_index that belong to the bump
    ## and convert new index list to matrix # ????? check
    temp = []
    for i in range(len(knn_index)):
        if knn_index[i][0] in partition.index:  # ????? knn_index[i][0] == i
            temp.append(knn_index[i])

    knn_matrix = np.array(temp)

    ## total number of new synthetic observations to generate
    n_synth = int(n * (perc - 1))
    ...
    for i in range(len(knn_matrix)):
        ...
        for j in range(1, k + 1):
            if label[
                knn_matrix[i][j]] == -1:  # ????? label sorted, knn_matrix[i][j] unsorted UPDATE: solved in adasyn.py
                count_majority += 1
        ri = count_majority / k
        r.append(ri)
    ...
    for i in tqdm(range(len(index)), ascii=True, desc="index"):
        if Gi[i] > 0:
            ...
            for l in range(1, len(knn_matrix[i])):
                if label[knn_matrix[i][l]] == 1:  # ????? not necessary  #if there is any minority in the neighbors
                    if knn_matrix[i][l] in index:    # And if that minority is also present in the partition
                        no_minority = False
            ...
            for j in range(Gi[i]):
                ...
                if no_minority == False:
                    ...
                    synth_matrix[num + j, 0:(d - 1)] = data.iloc[index[i], 0:(
                                d - 1)] + rd.random() * diffs  # ????? SMOGN: boundary 1 excluded
        ...
        for j in range(len(feat_const)):
            data_new.insert(
                ...
                value=np.repeat(
                    ...
                    # ????? data or data_orig? UPDATE: changed from data to data_orig, may be problematic to some datasets, need further verification.
                    len(final_matrix))
            )
    ...
    # spark_data_new = SPARK.createDataFrame(data_new)
    return data_new
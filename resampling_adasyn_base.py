...
def run_adasyn(label_col, train):
    print(f"Running Vanilla ADASYNR base")
    try:
        ...
        print(f"Found exception in ADASYNR base : {e}")
...
def orchestrate_resampling(dataset_name):
    ...
    pd.DataFrame(EXECUTION_TIME).to_csv(f"{RESULT_EXECUTION_TIME_DIR}/execution_time_adasyn_base_{dataset_name}.csv", index=True)
    return
